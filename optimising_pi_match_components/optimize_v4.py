...
# Does the same as the function expected values, execpt it assumes the best distrubtion is a polynomial fit.
...
# Run the code, that takes the switch range, the amount of switches and the series.
def run(cap_range, switch_count, series):

    fig, ax = plt.subplots()
    ax2 = ax.twinx()

    # Expected values list, which are the values expected for each state.
    expected = expected_values(cap_range, switch_count)

    # Making a matrix of equations for the bvls to solve.
    coefficient_matrix = [np.array(x).flatten() for x in list(itertools.product([[0, 1], [1, 0]], repeat=switch_count))]

    # Do BVLS of the coeffcient matrix against the expected value vector.
    ...
    # Find the nearest capacitors to the solution found by BVLS.
    ...
    # Print the values calculated to terminal
